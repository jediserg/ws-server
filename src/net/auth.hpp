...
template <class Connection, class Policy> class Auth
{
public:
	...
	Auth()
	{
		HandlerManager::instance().addHandler("register", [this](Session::Ptr session, const json11::Json& json, HandlerCallback&& callback){
				callback(this->startAuth(session));
			});

        HandlerManager::instance().addHandler("login", [this](Session::Ptr session, const json11::Json& json, HandlerCallback&& callback){
